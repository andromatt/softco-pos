...
@section('content')
    ...
    <div class="container-fluid px-4">
        <h1 class="mt-4 text-center text-cafe-oscuro">Empleados</h1>

        <x-breadcrumb.template>
            <x-breadcrumb.item :href="route('panel')" content="Inicio" />
            <x-breadcrumb.item active='true' content="Empleados" class="text-cafe-oscuro" />
        </x-breadcrumb.template>

        @can('crear-empleado')
            <div class="mb-4">
                <a href="{{ route('empleados.create') }}">
                    <button type="button" class="btn btn-accion">Añadir nuevo registro</button>
                </a>
            </div>
        @endcan
        ...
        <div class="card">
                                ...
                                <td>
                                    @if ($item->img_path)
                                        <img class="img-thumbnail img rounded mx-auto d-block"
                                            src="{{ asset($item->img_path) }}" alt="{{ $item->razon_social }}">
                                    @else
                                            ...
                                            <ul class="dropdown-menu text-bg-light" style="font-size: small;">
                                                ...
                                                @can('editar-empleado')
                                                    <li><a class="dropdown-item"
                                                            href="{{ route('empleados.edit', ['empleado' => $item]) }}">Editar</a>
                                                    </li>
                                                @endcan
                                            </ul>
                                        </div>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form action="{{ route('empleados.destroy', ['empleado' => $item->id]) }}"
                                                method="post">
                                                @method('DELETE')
                                                @csrf
            ...
            <div class="card-footer">
                <form action="{{ route('import.excel-empleados') }}" method="post" enctype="multipart/form-data"
                    class="mb-3">
                    @csrf
                    <div class="mb-3">
